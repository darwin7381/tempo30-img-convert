...
def universal_intelligent_generate(image: Image.Image, context: dict) -> Image.Image:
    """萬能智能生成（極簡流程 + 超詳細萬能 Prompt）
    
    這是完全獨立的處理體系：
    - 極簡流程：不檢測身體範圍，不做複雜分析
    - 超詳細 Prompt：包含所有情況的處理指令（比原版更詳細）
    - 讓 AI 完全智能判斷：自動處理照片/插畫，自動裁切/生成
    - 一次 API 調用搞定一切
    """
    import os
    from dotenv import load_dotenv
    ...
    response = client.models.generate_content(
        model=API_CONFIG.model_image,
        contents=[
            UNIVERSAL_INTELLIGENT_PROMPT,  # 萬能智能 Prompt（超詳細）
            types.Part.from_bytes(data=img_bytes, mime_type="image/png")
        ],
        config=types.GenerateContentConfig(response_modalities=['TEXT', 'IMAGE'])
    )
